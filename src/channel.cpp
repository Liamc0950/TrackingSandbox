...
Channel::Channel(const int number, const int home_value) : m_number(number),
                                                      m_value(home_value),
...
void Channel::print_move_instructions() const {
    const MoveInstruction* current_move = m_first_move;
...
void Channel::insert_move_in_timeline(MoveInstruction *new_move) {
    const int target_cue = new_move->get_cue_number();
...
void Channel::remove_move_from_timeline(const MoveInstruction *move) {
...
int Channel::get_value_at_cue(const int cue_number) const {
    const MoveInstruction *current = m_first_move;
    const MoveInstruction *last_valid = nullptr;
...
MoveInstruction *Channel::get_next_change_after_cue(const int cue_number) const {
    const MoveInstruction *current = m_first_move;
    const MoveInstruction *last_valid = nullptr;
...
MoveInstruction *Channel::get_last_change_before_cue(const int cue_number) const {
    MoveInstruction *current = m_first_move;
    MoveInstruction *last_valid = nullptr;
...
bool Channel::has_move_at_cue(const int cue_number) const {
    const MoveInstruction *current = m_first_move;
